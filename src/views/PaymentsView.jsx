...
export default function PaymentsView({
  ...
  const creditItems = useMemo(() => creditLines
    ...
    .map(cl => ({
      ...
      id:           cl.id,
      nombre:       cl.nombre,
      sub:          cl.banco ? `${cl.banco} · ${cl.pagoMinimo ? 'Pago mínimo' : 'Saldo actual'}` : (cl.pagoMinimo ? 'Pago mínimo' : 'Saldo actual'),
      amount:       cl.pagoMinimo ?? cl.usado ?? 0,
      amountColor:  'var(--bad)',
      dueDay:       cl.fechaPago ?? null,
      color:        '#ef4444',
      tipoLabel:    'Tarjeta',
      saldo:        cl.usado ?? 0,
      defaultAmount: cl.pagoMinimo ?? cl.usado ?? 0,
      _raw: cl,
    }))
    .sort((a, b) => (a.dueDay ?? 99) - (b.dueDay ?? 99)),
  [creditLines])
